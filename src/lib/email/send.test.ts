...
describe("Resend contact sender", () => {
  ...
  it("uses the SDK idempotency option and branded React payload rather than custom headers", async () => {
    ...
    query.mockResolvedValue([{ normalized_email: "a@example.com", normalized_full_name: "Asha", normalized_phone: "+919876543210", normalized_enquiry: "A detailed project request", resolution_state: "resolved" }]);
