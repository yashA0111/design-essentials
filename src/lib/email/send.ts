...
export async function sendContactEmail(kind: "acknowledgment_email" | "internal_notification", enquiryId: string) {
 ...
 const { getSql } = await import("@/lib/db/client"); const rows = await getSql()<{ normalized_email: string; normalized_full_name: string; normalized_phone: string; country_code: string; normalized_enquiry: string; resolution_state: string }[]>`SELECT normalized_email, normalized_full_name, normalized_phone, country_code, normalized_enquiry, resolution_state FROM enquiries WHERE id = ${enquiryId}::uuid`; const enquiry = rows[0]; if (!enquiry) throw new JobFailure("ENQUIRY_NOT_FOUND", false);
 const isAck = kind === "acknowledgment_email";
 const payload = isAck ? { from: config.resendFrom, to: enquiry.normalized_email, subject: "We've received your inquiry — Design Essentials", react: ContactConfirmationEmail({ name: enquiry.normalized_full_name }) } : { from: config.resendFrom, to: config.internalRecipient, subject: `New Enquiry — ${enquiryId}`, react: InternalNotificationEmail({ fullName: enquiry.normalized_full_name, email: enquiry.normalized_email, phone: enquiry.normalized_phone, countryCode: enquiry.country_code, enquiry: enquiry.normalized_enquiry, enquiryId, identityConflict: enquiry.resolution_state === "identity_conflict" }) };
